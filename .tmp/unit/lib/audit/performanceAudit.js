...
exports.calcPerformanceScore = calcPerformanceScore;
...
function getNumberProp(node, key) {
    const value = node.props[key];
    return typeof value === "number" && Number.isFinite(value) ? value : null;
}
function runPerformanceAudit(tree) {
    const issues = [];
    const nodes = getAllNodes(tree);
    const images = nodes.filter((node) => node.type === "image");
    const freeNodes = nodes.filter((node) => node.props.positionMode === "free");
    const animatedNodes = nodes.filter((node) => {
        const animation = node.props.motionAnimation;
...
function calcPerformanceScore(issues) {
