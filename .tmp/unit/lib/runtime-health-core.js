...
exports.buildRuntimeHealth = buildRuntimeHealth;
function validateNextAuthUrl(url, env = process.env) {
    if (!url)
        return "NEXTAUTH_URL no esta configurada.";
    try {
        const parsed = new URL(url);
        const hasUnexpectedPath = parsed.pathname !== "/" && parsed.pathname !== "";
        const looksOutdatedLocalUrl = env.NODE_ENV === "development" &&
...
function validatePublicUrlAlignment(env = process.env) {
    const configured = [
...
function validateHttpsOrigins(env = process.env) {
...
async function buildRuntimeHealth(deps) {
    var _a;
    const services = [];
    const warnings = [];
    services.push({
    ...
    const authSecret = (_a = deps.env.NEXTAUTH_SECRET) !== null && _a !== void 0 ? _a : deps.env.AUTH_SECRET;
    ...
    if (nextAuthWarning)
        warnings.push(nextAuthWarning);
    const publicUrlAlignmentWarning = validatePublicUrlAlignment(deps.env);
    if (publicUrlAlignmentWarning)
        warnings.push(publicUrlAlignmentWarning);
    warnings.push(...validateHttpsOrigins(deps.env));
    warnings.push(...billing.checks
        .filter((item) => item.status === "warning")
        .map((item) => `${item.label}: ${item.detail}`));
    if (deps.env.NEXT_PUBLIC_BASE_PATH) {
        warnings.push(`NEXT_PUBLIC_BASE_PATH esta configurado como "${deps.env.NEXT_PUBLIC_BASE_PATH}", pero next.config.ts no define basePath.`);
    }
