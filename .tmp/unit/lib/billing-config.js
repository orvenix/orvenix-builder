...
exports.getBillingConfigReport = getBillingConfigReport;
function hasValue(value) {
    return Boolean(value && value.trim() && !value.includes("placeholder"));
}
function hasStripePrice(env, plan, interval) {
...
function warning(key, label, condition, detail) {
    return {
        key,
        label,
...
function stripeModeFromValue(value) {
    if (!hasValue(value))
        return null;
...
function normalizeUrl(url) {
    ...
    try {
        ...
        return (_a = url === null || url === void 0 ? void 0 : url.trim()) !== null && _a !== void 0 ? _a : null;
    }
}
function formatStripeMode(mode) {
    if (mode === null)
        return "sin definir";
    if (mode === "unknown")
        return "desconocido";
...
function buildStripeModeDetail(publishableMode, secretMode) {
...
function buildUrlAlignmentDetail(authUrl, nextAuthUrl, publicApiUrl) {
    const configured = [
...
function getBillingConfigReport(env = process.env) {
    ...
    const stripeModesAligned = stripePublishableMode !== null &&
    ...
    const configuredUrls = [authUrl, nextAuthUrl, publicApiUrl].filter((value) => Boolean(value));
    ...
    const checks = [
        ...
        ...stripePriceChecks,
        warning("stripe_mode_alignment", "Stripe modo consistente", !hasValue(env.NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY) || !hasValue(env.STRIPE_SECRET_KEY) || stripeModesAligned, buildStripeModeDetail(stripePublishableMode, stripeSecretMode)),
        ...
        warning("app_url_alignment", "URLs base alineadas", configuredUrls.length <= 1 || urlsAligned, buildUrlAlignmentDetail(authUrl, nextAuthUrl, publicApiUrl)),
    ];
    const stripeCoreOk = hasValue(env.NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY) &&
    ...
    const status = stripeCoreOk ? (hasWarnings ? "warning" : "ok") : mercadoPagoOk ? "warning" : "missing";
    return {
        status,
