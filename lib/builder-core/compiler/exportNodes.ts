import type { SitePageListItem } from "../tree/sitePages"
import { resolveRuntimeHref } from "../tree/pageLinks"
import { resolveSiteNavPages } from "../tree/siteNavigation"
...
export interface ExportResolvedNavLink {
  slug: string
  name: string
  href: string
  isActive: boolean
...
export interface ExportResolvedNavModel {
  ariaLabel: string
  navClass: string
  links: ExportResolvedNavLink[]
}
...
const COMPLEX_BLOCK_META: Record<string, ComplexBlockMeta> = {
...
export function getComplexBlockMeta(type: string) {
  return COMPLEX_BLOCK_META[type] ?? null
}

export function resolveExportHref(siteId: string | null | undefined, href: unknown) {
  return resolveRuntimeHref(siteId ?? null, href, "export")
}

export function resolveExportNavModel(
  props: Record<string, unknown>,
  options?: ExportSerializeOptions
): ExportResolvedNavModel {
  const navPages = resolveSiteNavPages(options?.pages ?? [], {
  ...
  return {
    ariaLabel: String(props.title ?? "Navegación"),
    navClass: `site-nav site-nav--${layout} site-nav--${justify} site-nav--${variant}`,
    links: navPages.map((page) => ({
      slug: page.slug,
      name: page.name,
      href: resolveExportHref(options?.siteId ?? null, `page:${page.slug}`),
      isActive: page.slug === (options?.currentPageSlug ?? "home"),
    })),
  }
}
