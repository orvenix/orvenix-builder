...
export async function getFunnelStepContext(
  ...
  const step = resolveCurrentFunnelStep(funnel.steps, { stepKind })
...
export async function getNextFunnelStepContext(
  ...
  const nextStep = resolveNextFunnelStep(funnel.steps, input)
...
async function getExactFunnelStepContext(siteId: string, funnelId: string, stepId: string) {
  ...
  const step = resolveCurrentFunnelStep(funnel.steps, { stepId })
  if (!step) return null
