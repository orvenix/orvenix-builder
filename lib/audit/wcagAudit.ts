...
export function runWcagAudit(tree: EditorTree): AuditIssue[] {
  ...
  nodes
    ...
    .forEach((node) => {
      const alt = String((node.props as Record<string, unknown>).alt ?? "");
  ...
  nodes
    ...
    .forEach((node) => {
      const label = String((node.props as Record<string, unknown>).label ?? "").trim();
      if (label.length === 0) {
        issues.push({
  ...
  const headingLevels = nodes
    ...
    .map((n) => Number((n.props as Record<string, unknown>).level ?? 2))
    .sort((a, b) => a - b);
  ...
  nodes
    .filter((n) => {
      ...
      const alt = (n.props as Record<string, unknown>).alt;
...
export function calcWcagScore(issues: AuditIssue[]): number {
  const wcagIssues = issues.filter((i) => i.domain === "wcag");
  const penalties = wcagIssues.reduce((sum, i) => {
    if (i.severity === "error")   return sum + 30;
    if (i.severity === "warning") return sum + 12;
    return sum + 4;
  }, 0);
