...
const navLinks = [
  { href: "/webs/inmobiliaria", label: "Inicio" },
  { href: "/webs/inmobiliaria/propiedades", label: "Propiedades" },
  { href: "/webs/inmobiliaria/agentes", label: "Agentes" },
  { href: "/webs/inmobiliaria/blog", label: "Blog" },
  { href: "/webs/inmobiliaria/contacto", label: "Contacto" },
]
...
const theme = {
  ...
  mobileBg: "#0d0a04", progressFrom: "from-amber-400", progressTo: "to-orange-500",
}

export default function InmobiliariaLayout({ children }: { children: React.ReactNode }) {
  return (
    <div className="min-h-screen bg-[#0d0a04] text-white font-sans">
      <EnhancedSiteNav brand={{ name: brand.name, sub: brand.sub }} links={navLinks}
        cta={{ href: "/webs/inmobiliaria/contacto", label: "Buscar propiedad" }} Icon={Building2} theme={theme} />
      <main className="pt-16">{children}</main>
      <footer className="border-t border-white/[0.04] bg-[#0a0702] py-14">
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-10 mb-10">
            <div>
              ...
              <p className="text-xs text-white/20 leading-relaxed mb-3">Bienes raíces premium en CDMX desde {brand.founded}. +$4.2B MXN en transacciones.</p>
            </div>
            <div>
              <h4 className="text-xs font-bold text-amber-700/60 uppercase tracking-wider mb-4">Propiedades</h4>
              <div className="space-y-2 text-xs text-white/25">
                {["Residencial","Departamentos","Locales Comerciales","Oficinas","Preventa","Renta"].map(s => <p key={s}>{s}</p>)}
              </div>
            </div>
            <div>
              <h4 className="text-xs font-bold text-amber-700/60 uppercase tracking-wider mb-4">Empresa</h4>
              <div className="space-y-2 text-xs text-white/25">
            ...
            <div>
              ...
              <div className="space-y-2 text-xs text-white/25">
                ...
                <p className="text-amber-900/50 pt-1">Lun–Sáb 9:00–19:00</p>
              </div>
            </div>
          </div>
