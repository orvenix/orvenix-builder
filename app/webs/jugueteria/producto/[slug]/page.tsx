...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const toy = juguetes.find((j) => j.id === slug)
...
export default async function ProductoPage({ params }: PageProps) {
  ...
  const toy = juguetes.find((j) => j.id === slug) ?? juguetes[0]
  const related = juguetes.filter((j) => j.id !== toy.id).slice(0, 4)

  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <div className="flex items-center gap-2 text-sm text-gray-500 mb-8">
        <Link href="/webs/jugueteria" className="hover:text-blue-600">
          Inicio
        </Link>
        <span>/</span>
        <Link href="/webs/jugueteria/catalogo" className="hover:text-blue-600">
          Catálogo
        </Link>
        <span>/</span>
      ...
      <div className="grid md:grid-cols-2 gap-12 mb-16">
        <div className="relative aspect-square bg-gray-50 rounded-3xl overflow-hidden">
          ...
          {toy.badge && (
            <span
              className={`absolute top-4 left-4 text-sm font-black px-3 py-1.5 rounded-xl ${badgeColors[toy.badge]}`}
            >
              {toy.badge}
            </span>
          )}
        ...
        <div>
          ...
          <p className="text-3xl font-black text-dark mb-6">
            ${toy.precio.toLocaleString('es-MX')}
          </p>

          <p className="text-gray-600 leading-relaxed mb-6">
            Un juguete diseñado para estimular la creatividad, la lógica y la
            imaginación de los chicos. Material certificado, seguro y pensado para
            horas de diversión.
          </p>

          <div className="flex gap-3 mb-6">
            <div className="flex items-center border-2 border-gray-200 rounded-xl">
              <button className="px-4 py-3 font-bold text-lg hover:bg-gray-50">
                -
              </button>
      ...
      <section>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-5">
          {related.map((j) => (
            <Link
              key={j.id}
              href={`/webs/jugueteria/producto/${j.id}`}
              className="toy-card block group"
            >
              ...
              <div className="p-3">
                <h3 className="font-black text-dark text-sm line-clamp-2">
                  {j.nombre}
                </h3>
                <p className="font-black text-blue-600 mt-1">
                  ${j.precio.toLocaleString('es-MX')}
                </p>
              </div>
            </Link>
          ))}
