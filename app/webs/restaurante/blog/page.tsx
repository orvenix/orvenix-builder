...
const categories = ["Todos", "Técnica", "Vinos", "Temporada", "Clases", "Postres", "Eventos"]

export default function RestauranteBlog() {
  ...
  return (
    <div className="min-h-screen bg-[#120b04]">
      <section className="relative py-24 overflow-hidden">
        <div className="absolute top-0 right-0 w-[500px] h-[400px] bg-amber-900/8 rounded-full blur-[120px]" />
        <div className="relative max-w-5xl mx-auto px-6 text-center">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-amber-900/20 border border-amber-700/25 text-amber-400 text-xs mb-6">Desde nuestra cocina</div>
          <h1 className="text-5xl md:text-6xl font-black mb-6">
            <span className="text-white">Blog</span>{" "}
            <span className="bg-gradient-to-r from-amber-400 to-orange-400 bg-clip-text text-transparent italic">gastronómico</span>
          </h1>
          <p className="text-white/35 max-w-xl mx-auto text-lg">Técnica, ingredientes, vinos y las historias detrás de nuestra cocina. Escrito por el equipo de Bella Terra.</p>
        </div>
      </section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filtered.map(post => (
            <article key={post.id} className="p-6 rounded-2xl border border-white/[0.06] bg-white/[0.015] hover:bg-white/[0.03] transition-all group cursor-pointer">
              ...
              <h2 className="font-bold text-white text-sm leading-snug mb-3 group-hover:text-amber-300 transition-colors italic">{post.title}</h2>
      ...
      <section className="py-16 bg-[#0a0502]">
        <div className="max-w-3xl mx-auto px-6 text-center">
          <p className="text-white/35 mb-6">¿La mejor manera de aprender gastronomía? Venirla a vivir.</p>
