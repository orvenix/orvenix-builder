...
import { deleteAutomation, isAutomationsReady, updateAutomation } from "@/lib/automation/runtime"
import {
...
export async function PATCH(req: Request, { params }: Ctx) {
  ...
  const body = UpdateAutomationSchema.safeParse(await req.json())
  if (!body.success) return NextResponse.json({ error: body.error.flatten() }, { status: 400 })

  try {
    const automation = await updateAutomation(siteId, id, body.data)
...
export async function DELETE(_req: Request, { params }: Ctx) {
  ...
  try {
    await deleteAutomation(siteId, id)
